...
mod server_test {
    ...
    async fn dataset_test() {
        ...
        let read_dataset = handler.read_entry_by_id::<DatasetEntry>(created_dataset.id.as_str()).await.unwrap(); 
        assert_eq!(read_dataset, created_dataset);


        handler.delete_dataset(created_dataset.id.clone()).await.unwrap();

        let value = handler.read_entry_by_id::<DatasetEntry>(created_dataset.id.as_str()).await.unwrap_err();
    ...
    async fn dataset_version() {
        ...
        let inserted_revision1 = handler.create_revision_for_group(&revision_request, created_object_group.id.as_str()).await.unwrap();
        let inserted_revision2 = handler.create_revision_for_group(&revision_request, created_object_group.id.as_str()).await.unwrap();
        ...
        let version = handler.create_datatset_version(&release_version_request).await.unwrap();
        let read_version = handler.read_entry_by_id::<DatasetVersion>(version.id.as_str()).await.unwrap();

        assert_eq!(version, read_version);

        handler.delete_dataset_version(version.id.clone()).await.unwrap();

        let read_version_error = handler.read_entry_by_id::<DatasetVersion>(version.id.as_str()).await.unwrap_err();
